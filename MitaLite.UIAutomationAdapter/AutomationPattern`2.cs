...
namespace System.Windows.Automation {
    internal class AutomationPattern<TManagedPattern, TComPattern> : AutomationPattern
        ...
        readonly Func<AutomationElement, TComPattern, TManagedPattern> wrap;

        internal AutomationPattern(
            int id,
            string programmaticName,
            Func<AutomationElement, TComPattern, TManagedPattern> wrap)
            : base(id: id, programmaticName: programmaticName) {
            this.wrap = wrap;
        }

        internal override object Wrap(AutomationElement element, object pattern) {
            return this.wrap(arg1: element, arg2: (TComPattern) pattern);
        }
    }
}